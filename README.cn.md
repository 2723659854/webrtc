...
- **编解码器固定为 H.264 + Opus**  
  为了最大化兼容主流浏览器、OBS 及常见移动设备，服务器目前仅支持 H.264 视频与 Opus 音频。  
  若业务侧需引入 VP8、VP9 或 H.265 等编码格式，需自行扩展 SDP 协商逻辑及 RTP 转发规则。

- **OBS 推流偶发自动断开**  
  在高动态画面或网络波动场景下，OBS 推流可能意外终止。  
  该问题与 libwebrtc 对 ICE consent、RTCP 反馈及带宽估计的严格要求有关，后续版本将持续优化 ICE 保活机制与 RTCP REMB 反馈以提升稳定性。  
  **临时缓解建议**：在 OBS 输出设置中启用 **CBR** 码率控制，并适当降低视频码率与分辨率。

- **OBS 编码器兼容性限制**  
  当前已验证可正常推流的 OBS H.264 编码器：
    - `x264`
    - `QuickSync H.264`
    - `AMD HW H.264 (AVC)`  
      这些编码器在 `baseline`、`main` 和 `high` 三个 Profile 下均可成功播放。  
      **不兼容的编码器**：`H264/AVC Encoder (AMD Advanced Media Framework)` —— 使用该编码器可能导致推流失败或画面异常。  
      如遇其他编码器问题，建议优先切换至上述已支持的编码器。
